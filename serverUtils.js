...
async function signinsOfDay(connection, day, granularity) {
  ...
  while (checkTime < curTime && isGymOpen(checkTime)) {
    console.log(
      `signinsOfDay(): minTime: ${checkTime.toISOString()}, maxTime: ${nextTime.toISOString()}`
    );
  ...
  if (checkTime > curTime && isGymOpen(checkTime)) {
    ...
    console.log(
      `signinsOfDay(today): minTime: ${checkTime.toISOString()}, maxTime: ${curTime.toISOString()}`
    );
...
async function signinsOfMonth(connection, year, month) {
  ...
  while (date.getMonth() == month) {
    ...
    console.log(
      `signinsOfMonth(): minTime: ${date.toISOString()}, maxTime: ${nextDate.toISOString()}`
    );
...
async function occupancyOfDay(connection, day, granularity, stayDuration) {
  ...
  while (checkTime < curTime && isGymOpen(checkTime)) {
    ...
    console.log(
      `occupancyOfDay: minTime: ${minEntryTime.toISOString()}, maxTime: ${checkTime.toISOString()}`
    );
  ...
  if (
    checkTime > curTime &&
    isGymOpen(checkTime) &&
    checkTime.toDateString() === curTime.toDateString()
  ) {
...
async function currentOccupancy(connection, duration) {
  ...
  console.log(
    `currentOccupancy: minTime: ${minEntryTime.toISOString()}, maxTime: ${curTime.toISOString()}`
  );
