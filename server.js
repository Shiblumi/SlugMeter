...
app.get("/signins/", async (req, res) => {
  try {
    ...
    const minInterval = 5, maxInterval = 600, defaultInterval = 60;
    let interval = parseInt(req.query.interval);
    if(isNaN(interval) || interval < minInterval || interval > maxInterval){
      interval = defaultInterval;
    }

    let responseText = "Today's estimated sign-ins sorted by " + interval + " minute intervals:<br><br>"
...
app.get("/occupancy", async (req, res) => {
  try {
    ...
    const minDuration = 5, maxDuration = 720, defaultDuration = 90;
...
async function queryCountInTimeframe(collection, startTime, endTime){
  const count = await collection
        .countDocuments({ timestamp: { $gte: new Date(startTime.toISOString()), $lte: new Date(endTime.toISOString()) } });
  return count;
}
